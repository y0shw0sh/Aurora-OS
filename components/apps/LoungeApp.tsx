...
export default function LoungeApp() {
  ...
  const sendMessage = async () => {
    ...
    setSending(true)
    await supabase.from('messages').insert({ user_id: userId, content: input.trim() })
    setInput('')
    setSending(false)
    stopTyping()
  }
        ...
        <>
          <div className="flex items-center justify-between pb-3 border-b border-white/10 mb-3">
            <div className="flex items-center gap-2">
              <div
                className={`w-2 h-2 rounded-full ${
                  othersOnline.length > 0 ? 'bg-emerald-400' : 'bg-white/20'
                }`}
              />
            ...
            <button
              ...
              className="px-3 py-1 rounded-lg bg-white/10 hover:bg-white/20 text-white/70 hover:text-white text-xs transition-colors"
            >
              📹 Start call
            </button>
          </div>
